...
\begin{definition}
    ...
    \lean{LHCP.Tseitin.T}
    \leanok
    \uses{Tseitin.Ns, BigAnd, NonAtomic}
...
\begin{lemma}
    ...
    \lean{LHCP.Ns_eq_empty_iff}
    \leanok
    \uses{NonAtomic, Tseitin.Ns}
    For any formula $f$, $N^*(f)$ is empty iff $f$ is atomic.
\end{lemma}

\begin{lemma}
    \label{not_nonAtomic}
    \lean{LHCP.not_nonAtomic}
    \leanok
    \uses{NonAtomic}
...
\begin{theorem}
    ...
    \lean{eval_eq_of_mem_ssub}
    \leanok
    \uses{Atoms, mem_atoms_of_subformula, neg_ssub, imp_ssub, eval_not, eval_imp}
